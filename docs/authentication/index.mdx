...
installation tokens, or the git-http provider — throw `unsupported`. Branch on `client.capabilities.userProfile` before calling
...
- **Scopes:** GitHub needs `user:email` (classic PATs / OAuth) or the "Email addresses" read
  permission (fine-grained PATs); Bitbucket needs the `email` scope.
- **A missing scope does not throw.** The emails call's `forbidden` error is swallowed and `email`
  stays unset — the email is enrichment, not a precondition. Any other failure (rate limit, network
  error) propagates as usual.
- GitLab, Azure DevOps, and Gitea already return the email on the profile call, so the flag is a
  documented no-op there. On GitHub the extra request is also skipped when the profile already
  carries a public email.

## Self-hosted and enterprise instances

Four of the six providers accept a `baseUrl` so you can target a self-managed deployment. Point it at
...
| Git HTTP     | Any git HTTP(S) remote   | no `baseUrl` — `repo` is the full remote URL |
...
<CardGroup cols={2}>
  ...
  <Card title="Git HTTP" icon="globe" href="/docs/authentication/git-http">
    Anonymous-first HTTP Basic auth against any plain git remote by full URL.
  </Card>
</CardGroup>

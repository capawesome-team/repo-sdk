...
| Gitea        | `owner/name`                               | —                     |
| Git HTTP     | full http(s) remote URL                    | —                     |
...
The git-http provider is the exception: it has no namespaces and no discovery, so `namespaces.list` and
`repos.list` throw `unsupported` — address a repository directly by its full remote URL.
...
<CardGroup cols={2}>
  ...
  <Card title="Discovering repositories" icon="folder-git" href="/docs/guides/repositories">
    This model in practice — list namespaces and repositories.
  </Card>
</CardGroup>

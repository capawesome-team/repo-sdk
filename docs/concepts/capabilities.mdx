...
repository search; only GitHub and GitLab emit `release` webhooks. Rather than hide these differences and
silently drop options, `repo-sdk` **advertises** each provider's capabilities and **gates** on them.

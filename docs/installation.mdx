---
title: Installation
description: Install repo-sdk, check runtime requirements, and learn the ESM subpath exports.
sidebar:
  label: Installation
  icon: package
---

Install `repo-sdk` from npm with your package manager of choice.

```package-install
repo-sdk
```

## Requirements

`repo-sdk` ships as **ESM only** and depends on nothing but the platform's `fetch` and Web Crypto APIs.

<TypeTable
  type={{
    Node: { type: 'string', default: '>= 20', description: 'Also runs on Cloudflare Workers and other Web-standard runtimes.' },
    Module: { type: 'string', default: 'ESM', description: 'Package "type": "module". No CommonJS build.' },
    Dependencies: { type: 'string', default: '0', description: 'Zero runtime dependencies.' },
  }}
/>

## Subpath exports

The client lives at the package root; each provider factory lives on its own subpath so you only pull in
the provider you use. Each provider subpath also exports the webhook helpers (`verifyWebhook`,
`parseWebhookEvent`) and a `commitWebUrl` helper for provider-specific commit URLs.

| Import                    | Exports                                                                 |
| ------------------------- | ----------------------------------------------------------------------- |
| `repo-sdk`                | `createClient`, `RepoError`, `detectWebhookProvider`, and all normalized types |
| `repo-sdk/github`         | `github`, `verifyWebhook`, `parseWebhookEvent`, `commitWebUrl`          |
| `repo-sdk/gitlab`         | `gitlab`, `verifyWebhook`, `parseWebhookEvent`, `commitWebUrl`          |
| `repo-sdk/bitbucket`      | `bitbucket`, `verifyWebhook`, `parseWebhookEvent`, `commitWebUrl`       |
| `repo-sdk/azure-devops`   | `azureDevOps`, `listOrganizations`, `verifyWebhook`, `parseWebhookEvent`, `commitWebUrl` |
| `repo-sdk/gitea`          | `gitea`, `verifyWebhook`, `parseWebhookEvent`, `commitWebUrl`           |
| `repo-sdk/git-http`       | `gitHttp` (no webhook helpers — git-http remotes emit no webhooks)      |
| `repo-sdk/testing`        | `createInMemoryProvider`                                                 |

:::note
`repo-sdk/testing` is meant for your test suite only — it provides a seedable in-memory provider so you
don't have to stub `fetch`. See [Testing](/docs/testing).
:::

## Import the client and a provider

```ts
import { createClient } from 'repo-sdk';
import { github } from 'repo-sdk/github';

const client = createClient({
  provider: github({ auth: { token: process.env.GITHUB_TOKEN! } }),
});
```

<Card title="Continue to the Quickstart" icon="zap" href="/docs/quickstart">
  Create a client, list commits, resolve a ref, and receive a webhook.
</Card>

...
token auth — see [Bitbucket authentication](/authentication/bitbucket).
:::
...
With an Entra ID `tokenProvider`, the credential is returned in `headers` instead of being embedded in
the URL. Pass it to git via `http.extraheader`:
...
<Card title="Next: Managing webhooks" icon="webhook" href="/guides/webhooks-managing">

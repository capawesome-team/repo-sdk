...
description: A practical guide to verifying webhook signatures from GitHub, GitLab, Bitbucket, Azure DevOps, and Gitea with one unified helper.
type: blog
date: 2026-07-18
---
...
| Gitea        | HMAC-SHA256  | `x-gitea-signature`                  | HMAC-SHA256 of the raw body as a bare hex digest, no prefix      |
...
Gitea's bare-hex encoding is the one that most often breaks code ported straight from GitHub; [How to Verify Gitea and Forgejo Webhook Signatures](/blog/verify-gitea-forgejo-webhook-signatures) covers that case on its own.

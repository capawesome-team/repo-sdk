...
description: A developer's comparison of the GitHub, GitLab, Bitbucket, Azure DevOps, and Gitea REST APIs — auth, pagination, rate limits, and webhooks.
type: blog
date: 2026-07-19
---
...
- **Gitea** wants its personal access tokens sent with the literal `token` scheme — `Authorization: token <pat>` — not `Bearer`, which it reserves for OAuth2-issued tokens. Forgejo behaves identically.
...
| Repo address         | `owner/name`                   | `group/.../project` or id        | `workspace/repo_slug`               | `project/repository`              | `owner/name`             |
| Pagination           | `Link` header                  | offset or keyset                 | `next` URL field                    | often single response             | `page`/`limit` + `Link`  |
...
If you are on Octokit today and weighing whether a unified layer is worth the swap, [Octokit Alternatives for Multi-Provider Git Integrations](/blog/octokit-alternatives-multi-provider) compares that path against maintaining one SDK per provider.
